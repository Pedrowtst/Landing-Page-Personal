...
assert.match(dumbbell3dJs, /DOCK_SETTLE_PROGRESS\s*=\s*0\.9\d+/, 'dumbbell waits for eased progress to settle near the endpoint before docking');
...
assert.match(dumbbell3dJs, /state\.docked = updateDockState\(state\.docked,\s*state\.progress,\s*state\.targetProgress\)/, 'dumbbell dock state is based on smoothed progress, not raw scroll reaching the endpoint');
...
assert.match(dumbbell3dJs, /function updateDockState\([\s\S]*wasDocked[\s\S]*DOCK_HOLD_PROGRESS[\s\S]*DOCK_SETTLE_PROGRESS/, 'dumbbell dock state has explicit attach and hold hysteresis');
