...
assert.match(html, /class="nav-overlay-close"/, 'mobile menu includes a visible close button inside the overlay');
assert.match(html, /<div class="nav-overlay-head">[\s\S]*class="nav-overlay-close"/, 'mobile menu close button sits in the visible overlay flow above the links');
assert.match(navOverlayCss, /\.nav-overlay-head\s*\{[\s\S]*display:\s*flex[\s\S]*justify-content:\s*flex-end/, 'mobile menu close row keeps the close button visible inside the panel');
const overlayCloseBlock = navOverlayCss.match(/\.nav-overlay-close\s*\{([^}]*)\}/)?.[1] || '';
assert.doesNotMatch(overlayCloseBlock, /position:\s*absolute/, 'mobile menu close button is not pinned to the hidden browser-chrome edge');
assert.match(overlayCloseBlock, /border-radius:\s*999px/, 'mobile menu close button remains a clean circular control');
assert.match(navMenuJs, /const closeButton = overlay\.querySelector\('\.nav-overlay-close'\)/, 'mobile menu close button is wired in JavaScript');
