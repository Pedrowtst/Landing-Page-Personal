...
assert.match(navOverlayCss, /\.nav-overlay-close\s*\{[\s\S]*position:\s*absolute[\s\S]*border-radius:\s*999px/, 'mobile menu close button is positioned as a clean circular control');
...
assert.match(navMenuJs, /panel\?\.focus\(\{ preventScroll: true \}\)/, 'mobile menu opens without visually marking Historia as focused');
