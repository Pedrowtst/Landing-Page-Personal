...
const navOverlayCss = fs.readFileSync('css/nav-overlay.css', 'utf8');
...
assert.doesNotMatch(html, /class="nav-overlay-close"/, 'mobile menu no longer has a separate close button; the hamburger morphs into the X close affordance');
assert.doesNotMatch(html, /<div class="nav-overlay-head">/, 'mobile menu drops the close-button row in favor of the unified hamburger toggle');
assert.doesNotMatch(navOverlayCss, /\.nav-overlay-close\s*\{/, 'mobile menu close button styles are removed alongside the unified toggle');
assert.doesNotMatch(navOverlayCss, /\.nav-overlay-head\s*\{/, 'mobile menu close-button row styles are removed alongside the unified toggle');
assert.doesNotMatch(navMenuJs, /querySelector\('\.nav-overlay-close'\)/, 'mobile menu no longer queries a separate close button; the hamburger drives both open and close');
const navZIndex = zIndexFor('.nav', navCss);
const navOverlayZIndex = zIndexFor('.nav-overlay', navOverlayCss);
assert.ok(navZIndex !== null && navOverlayZIndex !== null && navZIndex > navOverlayZIndex, 'navbar (with hamburger) sits above the mobile menu overlay so the toggle stays clickable while open');
assert.match(navCss, /\.nav-hamburger\[aria-expanded="true"\]\s*\{[\s\S]*border-radius:\s*999px/, 'hamburger morphs into a circular close affordance when the menu is open');
assert.match(navCss, /\.nav-hamburger\[aria-expanded="true"\] \.nav-hamburger-bars\s*\{[\s\S]*transform:\s*rotate\(180deg\)/, 'hamburger bar container rotates a half-turn while the bars cross into an X for a unified morph');
assert.match(navMenuJs, /classList\.add\('nav-open'\)[\s\S]*classList\.remove\('nav-open'\)/, 'mobile menu locks body scroll while open and releases it on close');
assert.match(navOverlayCss, /body\.nav-open\s*\{[\s\S]*overflow:\s*hidden/, 'mobile menu CSS locks body scroll while the overlay is open');
assert.match(navOverlayCss, /html\.nav-open\s*,?[\s\S]*body\.nav-open\s*\{[\s\S]*touch-action:\s*none/, 'mobile menu CSS also blocks touch-action scroll gestures while open');
assert.match(navOverlayCss, /\.nav-overlay-list a\s*\{[\s\S]*pointer-events:\s*auto/, 'mobile menu links explicitly enable pointer events so taps register even with the panel pointer-events transparent');
assert.match(navOverlayCss, /\.nav-overlay-cta\s*\{[\s\S]*pointer-events:\s*auto/, 'mobile menu CTA explicitly enables pointer events so taps register even with the panel pointer-events transparent');
assert.match(navMenuJs, /const panel = overlay\.querySelector\('\.nav-overlay-panel'\)/, 'mobile menu focuses the dialog panel, not the first menu link');
assert.match(navMenuJs, /function shouldMoveFocus\(\)[\s\S]*is-keyboard-modality/, 'mobile menu only moves focus when opened through real keyboard navigation');
assert.match(navMenuJs, /function clearOverlayLinkState\(\)[\s\S]*removeAttribute\('aria-current'\)[\s\S]*blur\(\)/, 'mobile menu clears any stale section selection from overlay links when it opens');
assert.match(navMenuJs, /function focusPanelIfKeyboard\(\)[\s\S]*shouldMoveFocus\(\)[\s\S]*panel\?\.focus\(\{ preventScroll: true \}\)/, 'mobile menu keeps dialog focus available for keyboard users only');
assert.doesNotMatch(navMenuJs, /focusables\[0\]\?\.focus\(\{ preventScroll: true \}\)/, 'mobile menu no longer auto-focuses the Historia link on open');
assert.match(navOverlayCss, /\.nav-overlay-list a\s*\{[\s\S]*-webkit-tap-highlight-color:\s*transparent[\s\S]*outline:\s*none/, 'mobile menu links do not show tap or default focus paint on touch open');
assert.match(navOverlayCss, /\.nav-overlay-list a:focus\s*\{[\s\S]*outline:\s*none[\s\S]*box-shadow:\s*none/, 'mobile menu links suppress non-keyboard focus chrome');
assert.doesNotMatch(navOverlayCss, /(?:^|\n)\.nav-overlay-list a:focus-visible\s*\{/, 'mobile menu does not style focus-visible globally for touch/browser focus quirks');
assert.match(navOverlayCss, /html\.is-keyboard-modality\s+\.nav-overlay-list a:focus-visible\s*\{[\s\S]*padding-left:\s*12px/, 'mobile menu keeps the focus treatment only for keyboard modality');
...
assert.match(scrollSyncJs, /visualViewport[\s\S]*resize[\s\S]*shouldTrackVisualViewportRefresh\(\)[\s\S]*requestViewportRefresh/, 'visual viewport resize refreshes native scroll measurements only for desktop zoom, not mobile URL-bar scroll noise');
assert.match(scrollSyncJs, /function shouldTrackVisualViewportRefresh\(\)[\s\S]*pointer:\s*fine/, 'native scroll sync ignores mobile visual viewport resize noise during touch scrolling');
...
const htmlBodyBlock = baseCss.match(/html,\s*body\s*\{([^}]*)\}/)?.[1] || '';
assert.doesNotMatch(htmlBodyBlock, /overflow-x:\s*hidden/, 'body is not turned into a competing horizontal-overflow scroll container');
...
assert.match(footerCss, /\.footer\s*\{[\s\S]*overflow:\s*clip/, 'footer glow is vertically clipped so it cannot create blank scroll space after the footer');
...
assert.match(dumbbell3dJs, /state\.scrollY = getPageScrollY\(\)/, 'dumbbell render loop uses the normalized page scroll source');
...
assert.match(dumbbell3dJs, /getDumbbellResponsiveParams\(getResponsiveViewportWidth\(\),\s*getRenderViewportHeight\(\)\)/, 'dumbbell presets are selected from the zoom-stable responsive width and cached render height');
...
assert.match(dumbbell3dJs, /antialias:\s*true/, 'dumbbell renderer keeps antialiasing enabled for cleaner model edges after DPR is capped');
assert.match(dumbbell3dJs, /responsiveWidth < 760\)[\s\S]*return Math\.min\(dpr,\s*1\.35\)/, 'mobile WebGL DPR is capped low enough to avoid Safari pinch-zoom crashes while keeping the dumbbell crisp');
...
assert.doesNotMatch(dumbbell3dJs, /SCROLL_SMOOTHING_ALPHA|smoothingAlpha|smoothProgress/, 'dumbbell progress is not frame-count smoothed because slow devices turn that into visible delayed chasing');
assert.match(dumbbell3dJs, /PROGRESS_NOISE_EPSILON\s*=\s*0\.0+/, 'dumbbell keeps only a tiny scroll-noise deadband, not delayed easing');
assert.match(dumbbell3dJs, /targetProgress[\s\S]*readScrollProgress\(state\.scrollY\)[\s\S]*resolveScrollProgress\(state\.progress,\s*state\.targetProgress\)/, 'dumbbell render progress is locked to current scroll progress with only noise suppression');
assert.match(dumbbell3dJs, /function resolveScrollProgress\([\s\S]*PROGRESS_NOISE_EPSILON[\s\S]*return target/, 'dumbbell progress jumps to the real scroll position instead of chasing it over many frames');
...
assert.match(dumbbell3dJs, /DOCK_HOLD_PROGRESS\s*=\s*0\.9\d+/, 'dumbbell keeps a small dock hold band so tiny reverse scrolls do not release and jump');
assert.match(dumbbell3dJs, /state\.docked = updateDockState\(state\.docked,\s*state\.progress,\s*state\.targetProgress\)/, 'dumbbell dock state is based on resolved progress, not raw scroll reaching the endpoint');
assert.doesNotMatch(dumbbell3dJs, /state\.docked\s*=\s*state\.targetProgress\s*>=\s*DOCK_ATTACH_PROGRESS/, 'dumbbell no longer teleports to the dock when native or page scroll jumps raw progress to 1');
assert.doesNotMatch(dumbbell3dJs, /DOCK_SETTLE_PROGRESS/, 'dumbbell docking no longer waits on delayed eased progress');
assert.match(dumbbell3dJs, /function updateDockState\([\s\S]*wasDocked[\s\S]*DOCK_HOLD_PROGRESS[\s\S]*DOCK_ATTACH_PROGRESS/, 'dumbbell dock state has explicit attach and hold hysteresis without delayed settling');
assert.match(dumbbell3dJs, /function getRenderViewportHeight\(\)[\s\S]*renderViewportHeight/, 'dumbbell projection uses a cached render viewport height instead of raw mobile URL-bar height noise');
assert.match(dumbbell3dJs, /if \(isUrlBarNoise\)\s*\{[\s\S]*lastViewportSignature = getViewportSignature\(\)[\s\S]*return;[\s\S]*\}/, 'mobile URL-bar resize noise is ignored before resizing the WebGL camera/canvas');
assert.match(dumbbell3dJs, /DOCK_ATTACH_PROGRESS\s*=\s*1/, 'dumbbell docks only at the completed endpoint');
assert.doesNotMatch(dumbbell3dJs, /DOCK_RELEASE_PROGRESS/, 'dumbbell avoids the old raw release threshold that caused canvas-boundary jitter');
...
assert.match(dumbbell3dJs, /state\.scrollY = getPageScrollY\(\)[\s\S]*state\.targetProgress = readScrollProgress\(state\.scrollY\)/, 'dumbbell reads scroll once per frame before computing progress');
assert.match(dumbbell3dJs, /const renderProgress = state\.docked \? 1 : state\.progress[\s\S]*renderFrame\(renderProgress\)/, 'docked dumbbell renders from the exact final pose so it cannot resize while latched');
assert.match(dumbbell3dJs, /function getDockedWorldPosition\([\s\S]*dockDocumentPoint[\s\S]*state\.scrollY[\s\S]*screenToWorldPoint/, 'docked dumbbell follows the document marker in world coordinates instead of switching CSS positioning');
assert.doesNotMatch(dumbbell3dJs, /function animate\(\)[\s\S]{0,120}syncMobileZoomSafety\(\)/, 'mobile zoom safety is event-driven instead of running matchMedia checks every animation frame');
...
const stageCanvasBlock = stageCss.match(/#dumbbell-stage canvas\s*\{([^}]*)\}/)?.[1] || '';
assert.doesNotMatch(stageCanvasBlock, /filter\s*:/, 'dumbbell canvas avoids full-viewport CSS filters that make mobile WebGL stutter on weaker GPUs');
...
assert.match(dumbbell3dJs, /params\.dockLift[\s\S]*getRenderViewportHeight\(\)/, 'docked dumbbell uses a small responsive lift from the explicit story marker');
...
assert.match(dumbbell3dJs, /function syncStageAttachment\(\)[\s\S]*stage\.style\.position = 'fixed'[\s\S]*stage\.style\.top = '0px'/, 'dumbbell canvas stays in one fixed coordinate system to avoid release teleporting');
assert.doesNotMatch(dumbbell3dJs, /stage\.style\.position\s*=\s*nextAttachment/, 'dumbbell no longer switches the canvas between fixed and absolute positioning');
assert.doesNotMatch(dumbbell3dJs, /syncStageAttachment\(state\.progress >= 1\)/, 'stage attachment is not driven by eased progress because that causes dock boundary jitter');
...
assert.match(buttonsCss, /\.btn\s*\{(?=[\s\S]*background:\s*transparent)(?=[\s\S]*isolation:\s*isolate)/, 'CTA buttons do not paint gradient directly on the transformed pill');
assert.match(buttonsCss, /\.btn-primary::before\s*\{[\s\S]*inset:\s*1px[\s\S]*background:\s*var\(--grad\)/, 'primary CTA gradient is painted on an inset inner layer');
assert.doesNotMatch(buttonsCss, /\.btn-primary\s*\{[^}]*background:\s*var\(--grad\)/, 'primary CTA no longer paints the transformed anchor itself');
assert.match(buttonsCss, /\.btn::after\s*\{[\s\S]*transform:\s*translateX\(-110%\)/, 'CTA shine is isolated from the main pill gradient layer');
...
assert.match(footerCss, /@media\s*\(max-width:\s*560px\)[\s\S]*\.footer\s*\{[\s\S]*padding:\s*clamp\(44px,\s*10vw,\s*64px\)\s+0\s+max\(24px,\s*calc\(env\(safe-area-inset-bottom\)\s*\+\s*24px\)\)/, 'mobile footer uses compact safe-area-aware padding instead of leaving a large blank tail');
assert.match(footerCss, /@media\s*\(max-width:\s*560px\)[\s\S]*\.footer-inner\s*\{[\s\S]*gap:\s*clamp\(28px,\s*8vw,\s*40px\)/, 'mobile footer internal gaps stay compact');
